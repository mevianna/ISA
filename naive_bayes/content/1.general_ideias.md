...
O Naive Bayes baseia suas previsões em probabilidades aprendidas dos dados de treinamento. Durante o treinamento, ele aprende as probabilidades das classes ocorrerem e as probabilidades de cada atributo ocorrer dentro de cada classe. Na classificação, ele calcula a probabilidade de uma nova instância pertencer a cada classe possível baseado em seus atributos, e escolhe a classe com maior probabilidade.[1]

### Exemplo: Classificação de Emails Spam

Para entender a intuição, vamos usar um exemplo clássico de classificação de emails. Imagine que, durante o treinamento com um grande conjunto de dados, o algoritmo observou o seguinte:
...
O Naive Bayes se baseia no **Teorema de Bayes** para calcular as probabilidades. Para entender o conceito, vamos começar com o caso mais simples, onde analisamos apenas **um atributo**, como um email que contém apenas a palavra "ganhe".
...
Agora, nosso objetivo é comparar as duas probabilidades para ver qual é a maior. Para classificar o email, precisamos determinar qual destes valores é maior: $P(\text{spam}|\text{"ganhe"})$ ou $P(\text{normal}|\text{"ganhe"})$.

Substituindo os valores que encontramos, a nossa tarefa é comparar:
$$\frac{0.24}{P(\text{"ganhe"})} \quad \text{com} \quad \frac{0.035}{P(\text{"ganhe"})}$$

Observe que o denominador $P(\text{"ganhe"})$ é o mesmo nas duas frações. Para descobrir qual fração é maior, podemos simplesmente comparar seus numeradores. A nossa comparação, então, se resume a: 
$$\text{Comparar } 0.24 \text{ com } 0.035$$

Como $0.24$ é maior que $0.035$, a pontuação da classe **SPAM** é maior, e o algoritmo classifica o email como tal.

>[!NOTE]
>Esta é a razão fundamental pela qual podemos ignorar o denominador $P(X)$ na prática da classificação. Como nosso objetivo é determinar qual classe tem a **maior** probabilidade, e o denominador é comum a todas elas, precisamos apenas comparar os numeradores.
...
- **A suposição "ingênua"**: Para contornar esse problema, o Naive Bayes assume que os atributos são independentes entre si. Isso significa que a presença de uma palavra não influencia a probabilidade de outra palavra aparecer. Matematicamente:
...
>
> - Por outro lado, as palavras "ganhe" e "dinheiro" em um email são, na realidade, **dependentes**; a presença de uma torna a outra mais provável
...
> [!IMPORTANT]
> A suposição de independência não é verdadeira em mutios problemas, no exemplo, a presença da palavra "ganhe" pode aumentar a chance de "dinheiro" aparecer, mas simplifica muito o algoritmo e funciona muito bem na prática.[2] O Nome **Naive** (ingênuo) para o algortimo vem dessa simplificação.
...
Como $0.168 > 0.0035$, o algoritmo classifica o email como **SPAM**, o que faz sentido intuitivo já que ambas as palavras são muito mais comuns em emails spam.

### Evitando Underflow: Log-Probabilidades

Na prática, quando há muitos atributos (ex: milhares de palavras), multiplicar probabilidades (valores entre 0 e 1) resulta em números extremamente pequenos, causando erros numéricos (underflow). Para resolver isso, trabalhamos com logaritmos das probabilidades ao invés das probabilidades diretas.

Como o logaritmo é uma função função **monótona crescente** (isto é, se a A é maior que B, então $\log(A)$ também será maior que $\log(B))$, encontrar a classe com maior log-probabilidade é o mesmo que encontrar a classe com maior probabilidade.
