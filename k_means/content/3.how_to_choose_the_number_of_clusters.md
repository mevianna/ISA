...
O coeficiente fica entre o intervalo de -1 a +1, em que "+1" representa a maior distância entre os clusters, "0" seria a aproximação do ponto de decisão e um valor negativo significa que, possivelmente, os dados estão agrupados de forma inadequada. Ele também pode ser utilizado como métrica de validação interna, para verificar a qualidade dos agrupamentos obtidos.

O cálculo do coeficiente de silhueta depende de duas medidas: a distância média intra-cluster (ou seja, entre um ponto e os demais pontos do mesmo cluster) e a distância média até o cluster mais próximo diferente do atual. No entanto, quando há apenas um cluster (k=1), não existe um segundo grupo com o qual comparar as distâncias externas. Assim, a distância ao cluster mais próximo não pode ser calculada, tornando o coeficiente de silhueta indefinido. Dessa forma, o método da silhueta não pode ser aplicado para k = 1.
